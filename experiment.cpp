...
AVLTree<int>* tree1 = new AVLTree<int>(); //insert odd numbers in ascending
order
AVLTree<int>* tree2 = new AVLTree<int>(); //insert odd numbers in descending
order
...
4.29 (500 nodes there) relatively
...
cout << "(EXP2.3) After inserting " << numIntegers << " random integers, AVL
tree size: " << avl->treeSize() << endl;
...
cout << "(EXP2.4) Compute height (DFS): " << avl->computeHeight() << "; root-
>height: " << avl->readRootHeight() << endl;
...
// use treeSize() to compute how many extra data we will insert into AVL
compared with Figure 4.29 (500 nodes there) relatively
int numIntegers = avl->treeSize();
double relativeSizeChangeFromTextbook = ( numIntegers - 500.0 ) / 500.0;
// use averageDepth() to compute what is the initial average depth before stage
2
double averageDepth = avl->averageDepth();
auto start = chrono::high_resolution_clock::now();
...
auto end = chrono::high_resolution_clock::now();
chrono::duration<double, milli> duration = end - start;
cout << "(EXP2.6) Random insert/remove " << numRandomInsertRemove << " random
integers. " << "Execution time: " << duration.count() << " ms" << endl;
...
cout << (-100*relativeSizeChangeFromTextbook) << "% less nodes" << endl;
cout << "(EXP2.9) Compute height (DFS): " << avl->computeHeight() << "; root-
>height: " << avl->readRootHeight() << endl;
cout << " testHeigth? " << testHeight(avl) << endl;
double newAverageDepth = avl->averageDepth();
double relativeChangeFromInitial = ( newAverageDepth - averageDepth ) /
newAverageDepth;
...
cout << " ==> average depth of my AVL tree does not change
significantly (<=1%)" << endl;
...
cout << " ==> my AVL tree improves the average depth over BST
significantly (>10%)" << endl;
}
}
